...
test('validateGlobOpts()', t => {
  ...
  const results = main({
    ...
    statCache: new Uint16Array(),
    symlinks: /.+/,
    ignore: new Set([null]),
    symlink: {}
  }).map(String);

  t.strictEqual(results.length, 8, 'should invalidate invalid glob options.');
  ...
  t.strictEqual(
    results[4],
    'TypeError: node-glob expected `statCache` option to be an object, but got Uint16Array [  ].',
  ...
  t.strictEqual(
    ...
    'true, false, \'FILE\', \'DIR\' or an array, but got an invalid value 1 in `/foo/4` property.',
  ...
  t.strictEqual(
    ...
    'but got an invalid string \'file\' in `/foo/5` property.',
  ...
  t.strictEqual(
    ...
    'but got an invalid value [Function: Function] in `/foo/2` property.',
  ...
  t.strictEqual(
    ...
    'but got an invalid object { dev: 3 } in `/foo/3` property, which doesn\'t have a valid file mode.',
  ...
  t.strictEqual(
    ...
    'but got an invalid value Symbol(false) in `/foo/1` property.',
